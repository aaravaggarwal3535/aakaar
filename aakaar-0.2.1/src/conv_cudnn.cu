...
std::shared_ptr<Tensor> run_cudnn_conv1d_backward_data(std::shared_ptr<Tensor> grad_y, std::shared_ptr<Tensor> w,
    ...
    if (it == g_bwd_data_cache.end()) {
        ...
        if (aakaar_cudnn_debug_enabled()) {
            fprintf(stderr, "[conv1d_cudnn_bwd_data] algo=%d time=%.4fms workspace=%zuMB\n",
            (int)perf_results[best].algo, perf_results[best].time, perf_results[best].memory / (1024*1024));
            fflush(stderr);
        }
...
std::shared_ptr<Tensor> run_cudnn_conv1d_backward_filter(std::shared_ptr<Tensor> x, std::shared_ptr<Tensor> grad_y,
    ...
    if (it == g_bwd_filter_cache.end()) {
        ...
        if (aakaar_cudnn_debug_enabled()) {
            fprintf(stderr, "[conv1d_cudnn_bwd_filter] algo=%d time=%.4fms workspace=%zuMB\n",
            (int)perf_results[best].algo, perf_results[best].time, perf_results[best].memory / (1024*1024));
            fflush(stderr);
        }
...
void run_cudnn_conv1d_forward_into(std::shared_ptr<Tensor> x, std::shared_ptr<Tensor> w, std::shared_ptr<Tensor> y,
    ...
    auto it = g_fwd_cache.find(key);
    if (it == g_fwd_cache.end())
        throw std::runtime_error("run_cudnn_conv1d_forward_into: no cached algorithm for this exact shape. "
            "Call run_cudnn_conv1d_forward() once, uncaptured, with matching x/w/y shapes first to warm the "
            "algorithm cache — this call refuses to allocate during capture.");
...
void run_cudnn_conv1d_backward_data_into(std::shared_ptr<Tensor> grad_y, std::shared_ptr<Tensor> w,
    ...
    int B = grad_x->shape[0], C_in = grad_x->shape[1], L_in = grad_x->shape[2];
    int C_out = w->shape[0], K = w->shape[2];
    int L_out = grad_y->shape[2];
    std::vector<int> key = {B, C_in, L_in, C_out, K, stride, padding, dilation, L_out};

    auto it = g_bwd_data_cache.find(key);
    if (it == g_bwd_data_cache.end())
        throw std::runtime_error("run_cudnn_conv1d_backward_data_into: algorithm cache not warm for this shape. "
            "Run an uncaptured backward pass first.");
...
void run_cudnn_conv1d_backward_filter_into(std::shared_ptr<Tensor> x, std::shared_ptr<Tensor> grad_y,
    ...
    int B = x->shape[0], C_in = x->shape[1], L_in = x->shape[2];
    int C_out = grad_w->shape[0], K = grad_w->shape[2];
    int L_out = grad_y->shape[2];
    std::vector<int> key = {B, C_in, L_in, C_out, K, stride, padding, dilation, L_out};

    auto it = g_bwd_filter_cache.find(key);
    if (it == g_bwd_filter_cache.end())
        throw std::runtime_error("run_cudnn_conv1d_backward_filter_into: algorithm cache not warm for this shape. "
            "Run an uncaptured backward pass first.");
...
void cudnn_set_stream_for_capture(std::shared_ptr<GraphHandle> handle) {
    cudnnStatus_t st = cudnnSetStream(CudnnManager::get_instance().handle, handle->stream);
    if (st != CUDNN_STATUS_SUCCESS)
        throw std::runtime_error(std::string("cudnn_set_stream_for_capture failed: ") + cudnnGetErrorString(st));
}

void cudnn_reset_stream() {
    cudnnSetStream(CudnnManager::get_instance().handle, 0);
}